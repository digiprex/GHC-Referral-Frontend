...
export default function WalletCards({ showHistory, user_data, customer_id,scrollToVouchers,setData,cashName }) {
  return (
    <>
      { customer_id && (user_data.number_of_pending_referrals || user_data.lifetime) ? <div className="walletCardsContainer">
        ...
        <div className="coinBalanceCardContainer container-dash-border">
          <CoinBalanceCard cashName={cashName} setData={setData} scrollToVouchers={scrollToVouchers} user_data={user_data} showHistory={showHistory}customer_id={customer_id} />
        </div>
      </div> : null }
    </>
  );
